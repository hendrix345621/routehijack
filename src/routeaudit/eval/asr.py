...
# prefer `answers_from_ids`, which segments via
...
def answers_from_ids(tokenizer, gen_ids: list[list[int]], spec=None):
    ...
    for ids in gen_ids:
        if spec.available:
            ...
            text = (
                tokenizer.decode(ids[anchor.answer_onset :], skip_special_tokens=True)
                if anchor.scoreable
                else ""
            )
        else:
            ...
            from ..model.thinking import TRUNCATED, Anchor
            ...
            anchor = Anchor(
                None if status == TRUNCATED else max(0, len(ids) - 1), (0, n_think), status, len(ids)
            )
            if status == TRUNCATED:
                text = ""
        answers.append(text)
...
def _last_real_logits(logits, attention_mask):
    """Select each padded sequence's final non-padding position."""
    import torch

    rows = torch.arange(logits.shape[0], device=logits.device)
    positions = torch.arange(attention_mask.shape[1], device=logits.device)
    ...
    with torch.no_grad():
        for i in range(0, len(prompts), batch_size):
            ...
            enc = tok(
                ...
                truncation=True,
                max_length=4096,
                add_special_tokens=(judge_kind != "llamaguard"),
            ).to(jdev)
            all_logits = judge(**enc).logits
            logits = _last_real_logits(all_logits, enc["attention_mask"])
